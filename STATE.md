...
## 27.08 — триаж тревоги shadow 44314803: гонка невыигрываема, соврал ПРИБОР

**Вердикт по гонке.** Победный tx стоит `idx=0` и везёт RedStone `ValueUpdate` по HYPE
(адаптер `0xe4ae88743c38`) В СЕБЕ — атомарный self-push, tip 40.0 gwei при base 3.579.
Реакцией не берётся по построению: между пушем и ликвидацией чужой tx не встанет. Денег не
потеряно.

**Дефект прибора (починен).** `in_hot=False` в тревоге — ЛОЖЬ: `our_view` снимается уже
ПОСЛЕ события, полная ликвидация обнуляет долг -> HF=inf -> `update_hotset()` выбрасывает
адрес. Поле систематически посмертное: на всей истории `in_hot=True` встречается 3 раза из
66. Опровергнуто тремя источниками — executor.log (жертва `0x85b98991` на кромке pre-arm
15:22:46–15:28:33, «⚡ armed» 59 раз, HF 1.0199->1.0016), архивный узел (блок 44314802:
collat $2317.45, debt $1654.23, **HF=1.0004**), и сходимость архивного долга $1654.23 с
`cover` события $1655.39 — она же позитивный контроль и на раскодирование жертвы, и на
архивность узла. Третий узел `READ_RPCS` контроль архивности ПРОВАЛИЛ (молча отдаёт latest
на исторический блок) — сначала он выдал debt=0, и вывод «жертва не та» был бы ложным.
Фикс: `bot/shadow.py` ведёт часы наблюдения hot-set, запись несёт `hot_age_sec` (когда
жертва в последний раз БЫЛА горячей) и `seen_horizon_sec` (глубина собственного наблюдения —
после рестарта «не видели» не значит «не вели»). Текст тревоги печатает возраст, не bool.
**В БОЮ вступит только после рестарта** — рестарт живого бота крон-сессия не делает.

**Побочная находка: суита врала на 8/8.** `if __name__ == "__main__": sys.exit(main())` стоял
в СЕРЕДИНЕ `bot/test_shadow.py`, а `main()` собирает `test_*` из `globals()` в момент вызова —
6 тестов, дописанных ниже (все `floor_ok` и все `upd_src`), не исполнялись НИ РАЗУ, а суита
рапортовала «8/8 passed». Блок перенесён в конец: теперь 14/14. Негативный контроль на новый
фикс сделан (снял штамп часов — тест падает).

**ЛИД (не мой скоуп, деньги): pre-arm не доводит котировку.** «quote error: hard deadline
8.0s exceeded» — по этой жертве 160 промахов против 59 армов (73%), по последним 5000 строк
pre-arm промах есть в **59.6%** строк. На ЭТУ потерю не влияло (атомарную гонку не берёт и
идеально армленный бот), но бьёт именно РЕАКТИВНЫЙ класс — единственный, где у нас бывают
победы. Причину не искал: горячий путь, отдельная сессия.

**Поправки в ~/.fleet-watch/INFRA-FACTS.md (обе — замером):** (1) «через `0x7a0ea1f7` ходят
ТРИ EOA» — не EOA, а поле `liquidator` события; отправитель ОДИН: `0x84d5e280903e`.
(2) «потолок разбора снят коммитом 2d105c0» — 2d105c0 его ВВЁЛ (`ANATOMY_MAX_TX=12`);
запись 44314803 идёт с `trunc=true` при `ntx=18`.
...
## 05.08 РАЗВЕДКА ЦЕПИ: мы чесали только HyperLend — рядом поток в 7.5 раза больше и БЕЗ замка

Вопрос владельца: есть ли рынки без доминаторов из белого списка. Прочёсан был только наш
протокол; сделан свип ВСЕЙ цепи за 30 сут (`tmp/sweep_chain.py`, 7 сигнатур ликвидационных
событий одним topic0-массивом, address=None, 864 лога за 76 мин).

**Позитивный контроль** (обязателен — HyperEVM молча обрезает getLogs): якорная ликвидация
HyperLend читается ТЕМ ЖЕ фильтром (address=None + массив топиков) и находится в полном
наборе; отдельный контроль на 30-дневном окне дал ровно 79 событий, как наш собственный скан.

### Что на цепи есть (30 сут)

| протокол | контракт | событий | приз | атомарных | победителей |
|---|---|---|---|---|---|
| **Morpho Blue** | `0x68e37de8…` | 721 | **$76,798** | **0/60** | 39, топ-1 68% |
| HyperLend (наш) | `0x00a89d7a…` | 79 | $10,247 | **9/9 = 100%** | 6, топ-1 доминирует |
| Aave-форк №2 | `0xcecce0eb…` | 88 | **$183** | 0/40 | 9, топ-1 28% |
| Euler v2 | `0xf73c654d…`,`0x28fca261…` | по 1 | — | — | — |

**Перекрёстная проверка** (`tmp/scan_neighbours.py`, независимый путь): контроль HyperLend
79 ✓, Morpho 695 транзакций ✓, тот же состав из 39 победителей ✓. Расхождение 695 vs 721 —
это tx против логов: в части транзакций больше одной ликвидации. Euler этим скриптом НЕ
измерен (упал на `invalid block range`) — число «2 события» взято из свипа цепи, не отсюда.

**Объёмный лидер ≠ денежный.** Топ-1 по ШТУКАМ `0x5e6d2d84…` — 328 событий (47%) на **$53**
(пылесос по AVLT/USD₮0). Топ-1 по ДЕНЬГАМ `0xa3f50477…` — 174 события на **$52,429** (68%),
второй `0x95a289ac…` — $20,758 (27%). То есть деньги делят ДВОЕ, и это не те, кто чаще всех
стреляет. Считать концентрацию по штукам здесь = обмануть себя вдвое.

**Morpho Blue на HyperEVM — в 7.5 раза больше денег, чем у нас, и структурно ОТКРЫТ.**
142 события дороже $10 (у нас 9). Атомарность мерена по-честному: есть ли в той же tx лог
оракула ЭТОГО рынка (из `idToMarketParams`) — ноль из 60 крупнейших. Контроль тем же кодом
на нашем пуле: 9/9 = 100%. То есть метод видит атомарность там, где она есть, а на Morpho
её нет: победитель реагирует на дрейф, а не двигает цену сам ⇒ это FCFS-гонка.
Рынки: AVLT/USD₮0 643 события (но почти вся мелочь), деньги в WHYPE/USDC, WHLP/USDHL,
kHYPE/USD₮0. Оценено 711/721 (99%), неоценённых 10 — печатаются, не глотаются.

**Замок белого списка — свойство HyperLend, а не цепи.** Свип `ValueUpdate` за 2 суток по
всей цепи: RedStone-пуш-адаптеров ровно ДВА, оба наши (A 322 пуша, B 1417). Ни один другой
лендинг на HyperEVM на них не сидит. Соседний Aave-форк: 0/40 ликвидаций через A/B.

**Второй Aave-форк `0xcecce0eb…`** (19 резервов, почти наш набор активов, ADDRESSES_PROVIDER
не отдаёт — не канон Aave): поле открытое (топ-1 28%, 9 победителей), но пустое — $183/мес.

### ЧЕСТНАЯ ГРАНИЦА ПОКРЫТИЯ

Свип видит только 7 перечисленных сигнатур. НЕ покрыты протоколы с собственным событием
ликвидации: **Felix CDP ($32M TVL)**, Hyperbeat, Hyperdrive, Sentiment, Parallel V3, Delpho,
Hyperstable, HypurrFi Isolated. Их надо разбирать отдельно — «не найдено» там означает
«не искали», а не «пусто».

### Урок дня (третий раз за сутки)

Смещение слова в `data` опять взято на глаз: ликвидатор в `LiquidationCall` — слово №2
(hex 128..192), а не 64..128; срез «по памяти» дал победителей вида `0x00000000…`. И первый
замер Morpho молча не оценил 673 события из 721 (наш AaveOracle не знает AVLT/WHLP) — вышло
бы «$1,609, денег нет» вместо $76,798. Доля неоценённого обязана печататься ВСЕГДА.

### Профиль поля Morpho: гонки за место в блоке ТАМ НЕТ (05.08)

«Открыт» ещё не значит «наш» — домерено, платят ли победители Morpho за скорость.
Три сигнала из чеков, все с контролем на нашем пуле (где поле заведомо жёсткое):

| | Morpho ≥$10 (80 шт) | HyperLend ≥$10 (9 шт) |
|---|---|---|
| чаевые, медиана | **0.101 gwei** | 2.454 gwei |
| чаевые, p90 | 0.360 | 2.950 |
| доля с tip ≤0.1 gwei | **49%** | **0%** |
| tx-index в блоке, медиана | 1 | 4 |

**Половина денежных побед на Morpho взята с чаевыми ≈ ноль.** За место в блоке там никто не
торгуется: победитель берёт добычу потому, что рядом никого нет, а не потому, что перебил
ставку. Контроль подтверждает, что метрика рабочая: на нашем пуле медиана в 24 раза выше и
ни одной победы с околонулевым tip. Максимум 161 gwei на Morpho показывает, что редкие
спорные моменты бывают, но это хвост, а не режим.

**ЧЕСТНЫЙ ПРЕДЕЛ ЭТОГО ЗАМЕРА:** «0 блоков с >1 ликвидацией» НЕ доказывает отсутствие
соперников — проигравший ревертит и лога `Liquidate` не оставляет, поэтому в чеках
победителей он невидим. Чтобы закрыть это, нужен отдельный скан РЕВЕРТНУВШИХ tx к Morpho
(не сделан). Вывод «поле не оспаривается» опирается на чаевые, а не на отсутствие соседей.

## 05.08 ПОПРАВКА К РАЗВЕДКЕ: «$76,798 у Morpho» — на 96% один вкладной токен

При премисс-чеке нового бота (репо `~/morpho-hl-liquidator`, «го» kelbic) заголовок
разведки не выдержал сверки со вторым источником:

| | on-chain (оракул рынка) | blue-api (свои цены) |
|---|---|---|
| 30 сут всего | +$76,786 | **−$2,040,457** |
| из них AVLT/USD₮0 | +$73,417 (96%) | −$2,042,992 |
| **БЕЗ AVLT** | **+$3,369** | **+$3,377** ✓ |

Источники расходятся в 27 раз ТОЛЬКО на AVLT (Altura Vault Tokens) и сходятся до 0.2% на
всём остальном — значит спор не о методе, а об одном вкладном токене с недоказанной
реализуемостью. Мой замер по `oracle.price()` был арифметически верен и стратегически
обманчив: оракул говорит, что Morpho КРЕДИТУЕТ, а не что мы ВЫРУЧИМ. Это тот самый класс,
который у нас уже записан (PHANTOM_MARKETS, урок kHYPE «не виден квотером = приз
недостижим»), — и я в него вошёл, процитировав его же двумя сообщениями ранее.

**Честная база Morpho (без AVLT): $3,318 за 30 сут / $111,936 за 90 сут** на событиях ≥$10.
Поток ломтями: деньги приходят стрессом. Вывод «слой открыт» СТОИТ (0/60 атомарных,
чаевые ≈0), но масштаб был завышен в ~23 раза.

**Урок в доктрину:** цена оракула — не цена выхода. Любой замер «сколько там денег» обязан
проходить вторым источником ИЛИ квотером до того, как станет заголовком.

## 07.08 Депег-вотч: два ночных подъёма на одной пиле — эскалационный гейт побудок

Тревоги 00:17Z и 02:17Z (кит 0x2385233a, HF=1.0250, съедено 38%→34%) — оба нижних зуба
ОДНОЙ шумовой полосы: отношение kHYPE/WHYPE с 05.08 ходит в 1.0186–1.0273 (~0.85%) с
периодом в часы, HF кита прыгает 1.0250↔1.0317. Вторая побудка пришла на МЕНЕЕ тяжёлой
картине, чем уже разобранная — новой работы не было.

Перемер 02:2xZ (всё по цепи, не по тексту тревоги):
- кит = чистая связка $280k kHYPE-залог / $237.7k WHYPE-долг (getUserReserveData) —
  ровно архетип вотча, его линейная модель HF×f для него точна, не консервативна;
- выход kHYPE→WHYPE: глубина до $500k при impact ~0, эффективная цена выхода ВЫШЕ
  оракульной на 0.36% (рынок не подтверждает оракульное сжатие — сжатие пилы живёт
  в оракуле; для нас это плюс к марже, cover ~$119k при 50% CF покрыт многократно);
- обрывы wstHYPE (~$65-70k), beHYPE (квотер-константа), PT (404) — прежняя база из
  карты глубины (#9), деградации нет;
- executor: цикл 1-3с, guard=OK, DRY_RUN=OFF, пре-арм держит кромку (2 позиции).

Правка вотча (analysis/depeg_watch.py): повторная побудка только по эскалации —
съедено выросло на ступень (HL_DEPEG_ESCALATION_STEP, 10 п.п. > наблюдаемой полосы
4 п.п.), сменился ближайший кит, или прошло HL_DEPEG_REARM_H=24ч (подавление с
сроком годности + суточное напоминание, пока эпизод длится). Состояние
(data/depeg_alert_state.json) НЕ сбрасывается на просветах: условие гасло в 01:17Z
между двумя побудками, сброс по просвету воспроизвёл бы спам на каждом нижнем зубе.
Гейт — чистая функция should_realert(), 8 тестов без транспорта
(test_depeg_watch.py). Попутно закрыт латентный краш: при пустом whales (hotset не
прочитался) notify падал на whales[0] и тревога о просадке терялась молча — теперь
будит с пометкой «китов в наборе НЕТ».

## 07.08 Вскрытие shadow-гонки 42516651: USDHL прайсится Pyth — атомарный self-push жив

Тревога `🏁 чужая ликвидация выше нашего пола: USDHL->USDT0 $363 (бонус ~$26)… in_hot=False`.
Разбор по цепи (не по тексту тревоги):

- **Не сбой.** Долг жертвы 0xc4c60bde = $362.96 < HL_MIN_DEBT_USD=500 → позиция сознательно
  вне hot-set И вне целей (пыльный фильтр). Полное закрытие (CF 100%, резервный долг <$2000),
  альтернатива «частичное 50% ⇒ полный долг $726 > фильтра» убита архивным чтением. За час до
  события HF висел 1.001–1.002 (чистая грань), дельта состояния: долг $362.96→0, залог
  $506.95→$114.83, seized $389.57 (реальная премия 7.2%, gross $26.3 — оценка сторожа сошлась).
- **Гонка была невыигрываема реактивно.** HF на ev-1 = 1.000305 (>1, позитивный контроль
  архивности: debt>0; ротация RPC подмешивает неархивные узлы — чтения с debt=0 до события
  отброшены как фолбэк на latest). Победитель 0xabcdebae сидел на idx 0 с tip 1.4 gwei и
  **вёз обновление цены Pyth в своей же tx** (первый лог = PriceFeedUpdate контракта Pyth
  0xe9d69cdd, топик 0xd06a6b7f сверен keccak'ом): атомарно пересёк HF<1 и сжал. Отдельного
  пуш-tx нет ⇒ push_idx пуст — эвристика сэндвича такие не видит по построению.
- **Оракульная гетерогенность венью (стратегическое).** getSourceOfAsset: USDHL =
  «Pyth-USDHL/USD» (0xa19b7fe6), USDT0 = «RedStone Price Feed» (0x61376924). Вывод 04.08
  «self-push мёртв (sender-гейт RedStone)» верен ТОЛЬКО для RedStone-активов; для
  Pyth-прайснутых (USDHL) пуш пермишенлесс — поле этим уже живёт. Наш spec-слой заточен под
  чужие RedStone-пуши и к Pyth-активам слеп; эквивалентный эдж = Hermes-фид + своя tx
  с апдейтом. Решение о такой стройке — за kelbic (капитал/пороги не трогал).
- **Счёт поля (5 теневых записей 04–07.08):** все жертвы в книге, все ниже фильтра $500;
  4 = пыль $2-4, одна $363. 0x6f7d45e5 взял 3/5 с tip 0.01, 0xabcdebae — idx-0 self-push
  с tip 1.4. Полоса $250-500: одна возможность ~$26 за 4 дня, оспорена предиктивным
  idx-0 стрелком — EV снижения HL_MIN_DEBT_USD мал и спорен; решение за владельцем.
- **Правка сторожа (bot/shadow.py):** (1) побудка агента теперь по ПОЛНОМУ полу огня
  (канон dd19a4e): debt≥MIN_DEBT И bonus≥MIN_PROFIT, None не будит — суб-фильтровые
  гонки остаются в jsonl, но не жгут триаж; (2) в анатомию каждой tx блока добавлен
  счётчик `upd` (Pyth PriceFeedUpdate + RS ValueUpdate внутри tx) — датасет
  predict-vs-react теперь различает атомарный self-push; (3) в `ours` добавлен
  `size_ok`. Тесты: bot/test_shadow.py (3), полный прогон 70 passed / 2 skipped.

## 08.08 Триаж депег-тревоги 12:17Z: суточный ре-арм, фактично, машина к киту готова

Тревога (кит 0x2385233a, HF=1.0257, съедено 30%) прошла гейт правомерно — 24ч ре-арм
(прошлая побудка 07.08 02:17Z); картина перемерена по цепи, не по тексту:
- отношение kHYPE/WHYPE = 1.01944 — очередной НИЖНИЙ зуб той же полосы 1.0185–1.0273,
  что с 05.08; дрейфа к воде НЕТ: HF кита на нижних зубьях стабилен трое суток
  (1.025–1.027), долг гасится ($243k → $232k);
- ПОЧЕМУ пила (новое): фид kHYPE — композит, description() =
  `kHYPE-prim:redstone/fundam-sec:chainlink/fundam-emerg:redstone/market`
  (0x18b82bb9…, источник из getSourceOfAsset). Гипотеза — переключение
  фундаментал↔маркет; НЕ доказана (аксессоры под-фидов закрыты, имплайд-курс DEX
  1.0226 = середина полосы, не дискриминирует), верхние зубья не монотонны — чистым
  курсом стейкинга полоса тоже не объясняется. Предел назван, не закрыт;
- пре-арм: кит в hot-set (198, опрос каждый цикл 1-4с), кромка PREARM_HF=1.02 — кит
  армится при сжатии ещё ~0.56%; движок жив, guard=OK, на кромке армлено 2 позиции
  (HF~1.0017); пороги в env не переопределены;
- глубина выхода: сейз при 50% CF = 2170 kHYPE (~$120.7k) → liqd, impact 0.002%,
  валовой бонус ~$5.2k — выход покрыт;
- нюанс метрики (не дефект, консервативно): eaten = худшая просадка по ВСЕМ LST
  против запаса ближайшего кита, а кит держит только kHYPE — 07.08 худшим был PT,
  которого у кита нет; числа тревоги могут не сходиться с позицией кита.

Правка вотча (аннотация, семантика гейта не тронута): assess() считает НИЗ окна и
пробой ниже него; статус и текст тревоги несут «внутри полосы окна / ПРОБОЙ низа —
новая территория». Низ окна дрейфует с полосой — это контекст разбора, не отбой;
решающей метрикой остаётся съеденный запас. +3 теста, полный прогон 229 passed /
2 skipped; живой прогон показал аннотацию и корректное подавление повтора.

## 2026-08-22 (крон-триаж): гонка 43838096 — $13.5k мимо, и это НЕ довод за дозалив газа

**Тревога.** hyperlend-shadow: чужая ликвидация WHYPE->kHYPE $164,450, бонус ~$14,800,
блок 43838096, tip 239.26 gwei, self-push x1, победитель 0xab7b11c6, жертва in_hot=True.

**Факт по цепи** (rpc.hyperliquid.xyz, tx `0x15210c12...`). Событие подтверждено дословно:
cover 2128.900052 WHYPE, seized 2241.748492 kHYPE, victim `0x2385233a`. Декод LiquidationCall
в записи честен — врут только производные.

**Промах был неизбежен по ДВУМ независимым причинам.**
1. *Класс закрыт структурно.* Первый лог tx победителя — адаптер RedStone `0xe4ae8874`,
   топик `0xf36866d9…` = TOPIC_VALUE_UPDATE, а LiquidationCall в ней 16-й. Это АТОМАРНЫЙ
   push+liq в одной tx. Push RedStone нам закрыт навсегда гейтом в байткоде (форк-канарейка
   04-05.08, 2478df5), сторонний слот HyperLend занят `0x7a0ea1f7` — и это ровно `to`
   tx-победителя (`from 0x84d5e280903e`). Реакция такую гонку не берёт ПО ПОСТРОЕНИЮ.
   Это те самые 28% атомарных из замера 20.08.
2. *Бот был разоружён ПО ЗАМЫСЛУ.* base блока 825.07 gwei; гард `bal >= GAS_LIMIT*2*base`
   требовал 4.13 HYPE против поплавка 0.156803 — в 26 раз. Ровно то поведение, под которое
   поплавок и выставлялся 20.08.

⚠️ **Этим событием НЕЛЬЗЯ аргументировать возврат капитала боту.** Соблазн силён: приз
~$13.5k против $141.92 всего потока за 14 суток в замере 20.08 — в ~95 раз больше всей
выборки, на которой обосновывали поплавок. Премиса «жирное приходит на 600-1100 gwei и
убыточно» этим случаем ДЕЙСТВИТЕЛЬНО поцарапана (газ тут 834976 × 1064.33 gwei = 0.889 HYPE
≈ $70 против приза $13.5k). Но деньги ушли в класс, который поплавком НЕ ПОКУПАЕТСЯ:
полный бак не дал бы нам этой сделки. Довод за дозалив должен опираться на РЕАКТИВНЫЕ
жирные гонки, а этот случай к ним не относится.

**Три дефекта телеметрии — все от того, что ЖИВОЙ КОД СТАРШЕ РЕПО.** Процесс `bot.executor`
pid 546590 поднят 2026-08-07 04:32:01 и не перезапускался 15 суток ⇒ 3d92d99 (04:35 07.08),
2d105c0 и 57a3b10 (22.08) в бою НЕ РАБОТАЮТ.
* **Бонус $14,800 — модель, не замер:** `164449.99*0.09` ровно (дефолт bonus_bps=11000).
  Твёрдая величина из события — **+5.30% в токенах**. По оракулу ~+8.02% ($13,479 на долге
  $168,118), но это цены на `latest`: узел отдал ОДИНАКОВЫЕ значения для 43838095/43838096/
  latest ⇒ он не архивный, долларовую цифру как замер не переиспользовать.
* **Анатомия блока смещена на 1:** idx позиционные (фикс «idx из квитанции» не в бою).
  По квитанции победитель на каноническом **idx=2**, не 3; узел бота включил системную tx
  HyperCore (gas 0, tip −825.07) нулевой позицией — 11 tx против 10 у официального узла.
  `push_idx=[5,10]` читать как `[4,9]`, и он ещё посчитан старой силуэтной эвристикой ⇒
  этому полю в записи не верить. `upd` — честный признак, он в бою есть, и он тут решил.
* **`floor_ok` не мог быть False** (ПОЧИНЕНО, см. ниже).

**Правка (телеметрия, денежный путь не тронут).** `_floor_ok(bal, base)` вместо
`(bal is not None and …) or None`: питоновский `False or None` даёт None, поэтому поле
НИКОГДА не принимало значение False — законный «газа не хватило» был неотличим от «баланса
нет». Разбор уткнулся ровно в это: величина, которая его закрывала, была стёрта в null.
Незнание обязано закрывать гард, но знание обязано его ОТКРЫВАТЬ: три состояния = три
значения. +2 теста (средний случай False на боевом поплавке 0.156803 при 825 gwei; плюс
негативный контроль К СУИТЕ — воспроизводит снятый дефект и проверяет, что тест его ловит).
Полный прогон 253 passed / 2 skipped.

**НЕ сделано намеренно (решение ведущей сессии, не крон-триажа):** рестарт бота — до него
ни один фикс телеметрии, включая этот, в бою не действует. Отдельным хвостом: в `state.json`
`last_heartbeat` не двигался с 07.08 при живом `passes=1110949` и свежем mtime — на этот
вердикт не влияет, но выглядит как второй след того же 15-суточного процесса.

## 2026-08-20 — вывод газового поплавка на Base (решение kelbic)

**Что сделано.** 0.88 HYPE с EOA бота `0x46345D0c…673E3` → главный кошелёк Base
`0xE2Aca8e68A…908Fe` через Relay (999→8453), скриптом `morpho-hl-liquidator/analysis/bridge_out.py`
(тот же путь, что 06.08). tx HyperEVM `0x1de9330a97e1a24d0be8d34439740c1e4f97a8a692802990b29a2d0d505eaaeb`,
status=1, gasUsed 24 812. **Приёмка по дельте баланса получателя**: +0.027481 ETH через ~12с,
ровно котировка. Источник 1.036807 → **0.156803 HYPE** (nonce 7→8), получатель 0.346173 → 0.373654 ETH.
Цена перехода: отдали $62.81, получили $62.07 (ETH $2262.20, Chainlink на Base) = **$0.74 / 1.18%**.

**Почему именно столько.** Замер 20.08 (скан LiquidationCall за 14 сут + журнал гонок бота
за 16 сут, оба с позитивным контролем):
* 32 ликвидации за 14 сут, весь бонус потока НА ВСЕХ участников — **$141.92**;
* атомарных (RedStone push+liq в одной tx, нам закрыто) 28%, реактивных 72%;
* прошедших наши пороги ($25 приза И $500 долга) — **одна** за 14 суток;
* по квитанциям победителей: обе жирные гонки 19.08 были **убыточны** (газ $81.75 при призе
  $55.75; газ $60.15 при призе $25.38). Жирное приходит на base fee 600-1100 gwei, тихий фон 11.5.
  Единственная прибыльная реактивная за 16 суток — 07.08, +$26.06 на газе 1.5 gwei (USDHL = Pyth,
  пермишенлесс pull, см. память redstone-push-slot-whitelist).

Отсюда поплавок: **0.1568 HYPE ≈ $11.19**. Бот вооружён по своему гарду (`bal ≥ GAS_LIMIT×2×base`)
пока base ≤ **31.4 gwei** — весь тихий фон с запасом ×2.7, и сам разоружается в штормах, где
сделка убыточна. Тревога LOW GAS молчит в тишине (порог 0.0005) и на умеренном шуме (0.05 при
10 gwei); в шторм даст один звонок в час — это ожидаемо, не дефект.

**Наблюдение к сведению (не чинено).** `_gas_balance_ok` читает баланс ТОЛЬКО через
`C.RPC_WRITE` и при ошибке **fails open** (осознанно: «flaky RPC must never block a fire»).
Побочный эффект: пока эндпоинт лимитирует, тревога о низком газе не может сработать вовсе —
поле `balance_hype` остаётся протухшим. Замеренная частота: ~8 падений/сутки из 144 проверок
(~5%) в норме; при насыщении эндпоинта — 4/4. Починка на ~4 строки: при ошибке `_rpc_write`
пробовать `Rpc(C.READ_RPCS)` и лишь потом fail-open. Требует рестарта боевого бота — за kelbic.

**Откат.** Обратный перевод тем же скриптом с главного кошелька не предусмотрен (скрипт
односторонний, DEST зашит); доливка — обычным переводом на `0x46345D0c…673E3` в HyperEVM.

## 2026-08-22 Вскрытие shadow-гонки 43827262: гонка была НАША, слой молчал по своему запасу

Тревога `🏁 чужая ликвидация выше нашего пола: wstHYPE->wstHYPE $18,455 (бонус ~$2,491)
блок 43827262 tip 0.1 gwei, победитель 0x2f18fc90; жертва in_hot=True`. Разбор по цепи
(текст тревоги не источник: он же и оказался частично неверен — см. §телеметрия).

### Факт по цепи

* **Событие настоящее.** Квитанция `0x3da783f8…`: status=1, эмитент лога LiquidationCall =
  POOL `0x00a89d7a…` (не омонимия венью — урок 21.08 «имя цели ≠ протокол»), user =
  `0xaf421e57…`, coll = debt = wstHYPE, cover 226.952081, seized 257.590612, ratio **1.13500**
  (бонус 15% минус 10% протокольной комиссии — сходится с реестром). Премия победителю —
  **30.638 wstHYPE = $2,491.5**, В ТОМ ЖЕ АКТИВЕ: свопа для выхода не требуется вовсе.
  Победитель — контракт `0x2f18fc90`, EOA `0xa8b4d313…`; cover = ровно 50% долга (close factor).
* **Позиция.** Залог wstHYPE 387.073314 + USDT0 421.62 + USDC 18,633.22 ($50,371.61),
  долг ЦЕЛИКОМ wstHYPE 453.904170 ($36,725.80), LT 0.7301, eMode 0.
* **Что утопило.** Блок 43827262 (ts 1787364885) шёл так: idx1 — пуш RedStone на адаптер
  `0xe4ae8874` от авторизованного релейера `0x2327c3cd`, один ValueUpdate, фид **HYPE**
  78.370100 → **78.764632 (+0.5039%)**; idx2 — ликвидация победителя, tip **0.097 gwei, НИЖЕ
  пуша (1.129)**; idx3 — ВТОРОЙ гонщик ТЕМ ЖЕ контрактом `0x2f18fc90` с другого EOA
  `0x4dc3b0e8`, тот же tip, status=0 (опоздал на один индекс). Это техника lag=0, под которую
  и строился spec-слой, и поле гоняет её веером EOA по одному контракту.
* **HF.** На 43827261 = **1.001465** (по протоколу; реконструкция состава дала то же число —
  позитивный контроль). По ценам блока 43827262 при состоянии до ликвидации = **0.999608**.
  Пересечение — 0.039% глубиной, внутри одного блока.

### Реактивный путь: проиграно ПО ПОСТРОЕНИЮ, не по скорости

Бот цель ВИДЕЛ и держал армленной: `⚡ armed 0xaf421e57… HF=1.0015 wstHYPE->wstHYPE
net=$+2,372.8` — последний арм в 02:14:39, ровно перед блоком. Оценка бота ($2,373) сошлась
с фактом ($2,491 валом). Ни одной итерации с HF<1 не существовало: цель пересекла воду и была
съедена внутри одного блока. Реакция по подтверждённому состоянию такие гонки не берёт —
это и есть класс предиктивного слоя.

### Предиктивный слой: почему молчал (замер, а не догадка)

`HL_SPEC_FIRE=1` в живом окружении процесса (pid 546590, `/proc/…/environ`, чтение поимённо);
вотчер жив — `data/relayer_tips.jsonl` пишется каждый пуш (последняя запись на разборе —
блок 43827746, минутами позже события). Актив в карте DRIVERS. Кэши тёплые. Девиация пуша
0.5039% ≥ `SPEC_MIN_DEVIATION` — окно ОТКРЫЛОСЬ бы.

Связывающий предел — **`SPEC_HF_FIRE=0.998`**. Формула plan() на измеренных входах даёт
HF_est = **0.999566** (цепь: 0.999608, ошибка −0.000042) — выше порога ⇒ `plan()` вернул
None, зонда не было, в журнале не появилось ни строки. Запас порога назван как «0.2% на
дрейф долей и погрешность масштабирования»; **замеренная погрешность оценщика на ВСЕХ десяти
пересечениях за 14 суток — максимум 0.000042, то есть запас в ~48 раз больше ошибки, против
которой стоит.** Порог из величины ПЛАНА, а не из свойства субъекта (память
gate-input-from-subject-not-plan).

Заодно опровергнута строка docstring'а слоя «позиция coll==debt самогасится ⇒ не стреляет»:
сокращение наступает только при f_c == f_d. Здесь f_c = 0.6217 (wstHYPE — 62% залога),
f_d = 1.0000 (весь долг) — общий пуш поднял долг ЦЕЛИКОМ, а залог на 62%. Формула это считала
верно всегда; неверен был комментарий, и он снят (код по этому признаку не выходил досрочно
и не должен). Регрессия: `test_plan_same_asset_does_NOT_self_cancel_when_shares_differ`.

### Перепись класса за 14 суток (`analysis/spec_gap.py census`)

48 ликвидаций пула. **36 реактивных** (HF<1 уже на B-1; почти все — пыль $0.14–$8),
**10 пересечений В БЛОКЕ**, 2 не прошли позитивный контроль состава (реконструированный HF
не сошёлся с протоколом; долги $482.51 и $491.75 — оба НИЖЕ `HL_MIN_DEBT_USD=500`, денежного
события они спрятать не могут; причина не закрыта — названный предел).

*Поправка к первому прогону: черновой скрипт кэшировал ликвидационный порог резерва ПО
АДРЕСУ, без блока, и молча переносил сегодняшний LT на события двухнедельной давности — из-за
этого 9 составов «не сходились» и 6 пересечений терялись в мусорной корзине. В инструменте
ключ кэша — (актив, блок); цифры ниже — с него. Тот же класс ошибки, что «живой источник
против копии в репо»: величина, у которой есть время, не имеет права кэшироваться без него.*

| блок | долг | HF B-1 → пуш | HF_est (ошибка) | гейт 0.998 | размер | взял |
|---|---|---|---|---|---|---|
| 43827262 | **$36,726** | 1.001465 → 0.999608 | 0.999566 (−4.2e-5) | **МОЛЧАНИЕ** | **прошёл** | 0x2f18fc90 |
| 43626345 | $174 | 1.002653 → 0.999364 | 0.999364 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43626003 | $141 | 1.003670 → 0.999049 | 0.999049 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43828610 | $115 | 1.005791 → 0.997734 | 0.997734 (−0.0e0) | огонь | ниже фильтра | 0xae86edb5 |
| 42901129 | $14 | 1.000005 → 1.000000 | 1.000000 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 42894450 | $14 | 1.000005 → 0.998700 | 0.998700 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43827324 | $8 | 1.003996 → 0.999565 | 0.999565 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43160650 | $7 | 1.004894 → 0.999604 | 0.999604 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43141999 | $7 | 1.004879 → 0.999736 | 0.999736 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |
| 43127904 | $7 | 1.003998 → 0.999980 | 0.999980 (−0.0e0) | МОЛЧАНИЕ | ниже фильтра | 0xae86edb5 |

**9 из 10 пересечений гейт 0.998 не пропускает**, и единственное, что он пропустил бы, —
$115, то есть ниже нашего же размерного фильтра. За 14 суток у слоя была **ровно одна**
денежная возможность (43827262, $2,491 премии), и её съел собственный запас. Это не «слой
мёртв» и не «поля нет» — это порог не по субъекту.

Погрешность оценщика по всем 10 пересечениям: максимум **0.000042** (на 43827262, единственной
смешанной позиции; на одноактивных формула точна до знака). Запас порога 0.998 — 0.2%, то есть
**в ~48 раз больше ошибки, против которой стоит**.

Попутно: 9 из 10 пересечений забрал ОДИН оператор `0xae86edb5` — он гоняет предиктивную
технику на пыли систематически; жирное 43827262 взял `0x2f18fc90` веером из двух EOA.
Класс не теоретический и не наш личный: у него есть постоянные жильцы.

### Вторая премисса слоя опровергнута замером (`analysis/spec_gap.py devs`)

`SPEC_MIN_DEVIATION=0.005` стоит на записи 04.08 «приняты 0.501/0.504/0.507/0.508%, ничего
ниже». Замер шага значения между соседними ValueUpdate адаптера A за 8ч: **136 пушей, 25
(18%) ниже 0.50%, минимальный ПРИНЯТЫЙ шаг 0.3533%** (HYPE 4/48, kHYPE_FUND 4/40, SOL 7/29,
UBTC 5/10, BTC 5/9). Значит у релейера есть не только девиационный, но и хартбит-повод, и
слой слеп к ~18% пушей ПО ПОСТРОЕНИЮ. Часть пересечений выше (шаги 0.33%, 0.44%, 0.46%) была именно такой — у них к порогу HF добавлялся ещё и порог девиации. Названный предел
замера: шаг on-chain = принятая девиация лишь при том, что гейтвей и chain совпадают сразу
после пуша (медиана пакетов против медианы гейтвея).

### Что сделано (в моей власти) и когда включится

Правки инертны до рестарта боевого процесса; **рестарт — за kelbic** (прецедент 20.08).

* `bot/shadow.py` — **push_idx считался силуэтом tx и промахнулся в обе стороны**: настоящий
  пуш (idx1, inb=581) не прошёл порог `inb>=600`, а помечен был посторонний idx4 (inb=1540,
  upd=0). Теперь push_idx = tx с РЕАЛЬНЫМ оракул-событием (`upd>0`) — тот же честный признак,
  которым 07.08 опознан атомарный self-push. Названный предел: видны RedStone ValueUpdate и
  Pyth PriceFeedUpdate — ровно те оракулы, которыми HyperLend прайсит резервы.
* `bot/shadow.py` — **индекс tx теперь из КВИТАНЦИИ, не из позиции в листинге блока.**
  Вскрытие: на 43827262 `rpc.hyperlend.finance` отдаёт **6** tx, а `rpc.hyperliquid.xyz/evm`
  и drpc — **5**: они опускают системную tx HyperCore (`0xf152e476…`, gasPrice 0, gasUsed 0,
  трансфер USDC), причём drpc в квитанциях всё равно нумерует каноnично (idx 3), а
  официальный узел перенумеровывает (idx 2). То есть `idx` датасета до сегодня был
  ЭНДПОИНТ-ЗАВИСИМ, а «idx 0 = верх блока» — неверно там, где на нуле сидит системная tx с
  tip ниже базы. Расхождение листинга и квитанций теперь живёт В ДАННЫХ (`idx_shift`), как и
  размер блока и факт усечения разбора (`ntx`, `trunc`, потолок 12 tx).
* `bot/shadow.py` — **бонус теперь ЗАМЕР, а не модель**: `seized_usd − debt_usd` вместо
  `debt × (bonus_bps−1) × 0.9`, с пометкой `bonus_src` и прежней моделью как фолбэком без
  цены залога. На этом событии обе величины совпали ($2,491.5) случайно — конфигурационный
  бонус и протокольная комиссия сошлись с прошитыми; на eMode/пыльных добиваниях не сойдутся.
  Это вход гейта побудки, поэтому цифра обязана быть замеренной (память
  acceptance-reference-not-self-report).
* `analysis/spec_gap.py` — новый инструмент (`census` / `devs`), которым обе таблицы выше
  перемеряются одной командой; у обоих режимов позитивный контроль, у census — явная печать
  того, что выпало из знаменателя (без молчаливого усечения).
* Тесты: `bot/test_shadow.py` 8/8 (фикстура — замеренная анатомия 43827262; прежняя
  эвристика держится в тестах негативным контролем), `bot/test_spec.py` 30/30. Полный прогон
  **242 passed / 2 skipped**. Негативный контроль прогнан ПО КАЖДОЙ ПОЛОВИНЕ фикса отдельно
  (снятый idx-из-квитанции и снятый push_idx роняют суиту независимо).

### Решения за kelbic (огневой путь — не трогал)

1. **`HL_SPEC_HF_FIRE`: 0.998 → ~0.9995.** Замер: запас в ~48 раз больше ошибки оценщика;
   9/10 пересечений за 14 суток промолчали, и единственное денежное из них стоило $2,491
   (то, что гейт пропустил бы, — $115, ниже размерного фильтра). Цена ошибки ограничена
   и измерена: зонд мимо ревертится на HF-чеке за ~211k газа (~$0.001 на тихом фоне), consec-
   счётчик kill-switch зонды не кормит, дневной газ-кап их считает.
2. **`HL_SPEC_MIN_DEVIATION`: 0.005 → ~0.0035.** 18% принятых пушей ниже нынешнего порога,
   минимальный 0.3533%. Плата — окна открываются чаще, значит больше зондов; потолок
   `SPEC_MAX_PROBES=12` на окно остаётся.
3. **Рестарт бота** — иначе ни правки телеметрии, ни любая из ручек выше не вступят в силу.

Обе ручки — самостоятельные откаты (вернуть прежнее значение). Капитал, пороги огня по
деньгам (`MIN_PROFIT`/`MIN_DEBT_USD`) и боевой .env я не трогал.

### Перепроверка вскрытия 43827262 независимой сессией (22.08 ~03:2x-03:3xZ, agent-triage)

Разбор выше делала сессия, которую в 02:50 убил таймаут (rc=124) через две минуты после
коммита: вывод в тревогу записан НЕ был, тревога осталась `handled=false` с `attempts=2`.
По правилу «разбор без записанного вывода = разбора не было» несущие факты перемеряны заново.

**Сошлось до знака:** квитанция `0x3da783f8…` — status=1, блок 43827262, **txIndex 2**,
to `0x2f18fc90…`, from `0xa8b4d313…`, 23 лога. HF жертвы на **B-1 = 1.0014651** (залог
$50,371.61, долг $36,725.80, LT 0.7301); после ликвидации — $29,305.68 / $18,455.43, HF 1.1482.
В `executor.log` серия `⚡ armed 0xaf421e57… HF=1.0015 wstHYPE->wstHYPE net=$+2,373` вплоть до
блока. Тесты на HEAD — **243 passed / 2 skipped**, `local == origin == 2d105c0`.

**Позитивный контроль архива понадобился и сработал.** Первый заход (`rpc.hyperliquid.xyz`,
плюс drpc и rpc.hyperlend.finance, отвалившиеся с 403 — запрос шёл без User-Agent) отдал на
B-1, B и latest **одно и то же** 1.1207: узел не архивный и молча подставляет latest на
исторический тег. Вывод из такого ответа был бы ложным в обе стороны. Годные цифры — только
со второго захода, архивным `hyperliquid.drpc.org` через клиент репо (он всегда шлёт UA),
где значения по блокам РАЗЛИЧАЮТСЯ. Тот же класс, что «форк-симуляция: не-архивный апстрим
даёт тихий неверный результат» — и он ловится ровно одним лишним чтением соседнего блока.

**Поправка к разделу «Решения за kelbic».** Формулировка «ручки боевого .env» описывает не тот
объект: `HL_SPEC_HF_FIRE` и `HL_SPEC_MIN_DEVIATION` в окружении живого процесса **не заданы**
(чтение `/proc/546590/environ` поимённо; задан только `HL_SPEC_FIRE=1` и `HL_MIN_DEBT_USD=500`).
0.998 и 0.005 приходят **дефолтами кода** — `bot/config.py:242,247`. Менять надо там либо
добавлять переменные в env; исполнителю это знать обязательно.

**Второе наблюдение — боевой shadow инертен не со вчера.** Живой процесс (`python3 -u -m
bot.executor loop`, pid 546590) стартовал **07.08 04:32:01**, а коммит `3d92d99` в `bot/shadow.py`
лёг **04:35:16** — на три минуты ПОЗЖЕ старта. Значит телеметрия, по которой сторож судит и
формулирует тревогу, в бою уже 15 суток отстаёт на один коммит, а `2d105c0` добавляет второй.
Это усиливает пункт 3 (рестарт): он не «чтобы включить ручки», он ещё и закрывает накопленный
разрыв между боевым процессом и репозиторием.

Эскалировано владельцу одним HIL (`shadow-spec-gate-43827262`): три решения — два порога и
рестарт. Огневой путь, капитал, боевой env и Шарлотт не трогались.

## 2026-08-22 (исполнение решения владельца по 43827262): пороги переставлены, бот перезапущен

### ⚠️ Названная ручка приёмку НЕ проходила — поставлено 0.9996, не 0.9995

Решение назвало `SPEC_HF_FIRE 0.998 -> 0.9995` и тут же приёмку: «цель класса 43827262
(HF_est~0.9996) проходит гейт». Эти два условия несовместимы. Гейт строгий
(`bot/spec.py:355`, `hf_est < C.SPEC_HF_FIRE`), а HF_est цели, посчитанный **настоящим
`spec.plan()`** на замеренных входах, равен **0.999565767** — и `0.999566 < 0.9995` ложно.
Буквальная ручка оставила бы слой молчать ровно на том событии, ради которого её открывают.

| порог | 43827262 | запас 1−T | в ошибках оценщика (4.2e-5) |
|---|---|---|---|
| 0.998 (было) | МОЛЧАНИЕ | 2.0e-3 | 47.6x |
| **0.9995 (названо в решении)** | **МОЛЧАНИЕ** | 5.0e-4 | 11.9x |
| 0.99958 | огонь | 4.2e-4 | 10.0x |
| **0.9996 (поставлено)** | **ОГОНЬ** | 4.0e-4 | 9.5x |

Выбрано 0.9996: это ровно то число, которое решение само называет в приёмке, и оно же даёт
«~10x над ошибкой 4.2e-5» из того же решения (9.5x). Это не моё суждение о риске, а
механическое применение критериев владельца — обе строки прогнаны параметром через
настоящий `plan()`, вывод получен прибором, а не рассуждением. **Вето — одна строка env
плюс рестарт**, дефолт кода намеренно оставлен 0.998.

**Названный предел выбора.** 0.9996 клирит цель всего на 3.4e-5 — это МЕНЬШЕ ширины ошибки
оценщика. Соседняя цель того же класса с другим составом может лечь выше порога и снова уйти
в молчание. И сама ошибка измерена на ОДНОЙ смешанной позиции: на девяти одноактивных формула
точна до знака, то есть «max 4.2e-5» опирается на выборку из одного. Подъём выше требует либо
замера на большем числе смешанных позиций, либо согласия платить бóльшим числом зондов.

### Что поставлено и чем проверено (эффект, а не команда)

* `HL_SPEC_HF_FIRE=0.9996`, `HL_SPEC_MIN_DEVIATION=0.0035` — **в env**, дефолты кода не тронуты
  (откат без коммита). `MIN_PROFIT`/`MIN_DEBT_USD`/капитал/`SPEC_MAX_PROBES=12` не трогались.
* Рестарт 05:33:01, pid 546590 -> **2092462** (снят по точному pid, не по подстроке cmdline;
  личность сверена по `/proc/<pid>/cwd`). В момент снятия `sent:{}` — ноль отправок в полёте,
  `consec_reverts=0`, guard=OK. Дедман (600с) против воскрешения cron'ом (≤60с) — не будился.
* **Пороги прочитаны ИЗ ПРОЦЕССА, а не из env-файла.** Стартовый баннер теперь их печатает:
  `spec: HF<0.9996, dev>=0.0035, tip 0.02 gwei, <=12 зондов/окно`. Плюс `/proc/2092462/environ`
  поимённо. «Env записан» и «процесс прочитал» — разные утверждения; между ними порядок
  запуска, `set -a`, опечатка в имени и процесс, стартовавший ДО правки (что и было 15 суток).
* Маркеры нового кода на диске: `idx_shift`, `bonus_src`, `_floor_ok` — присутствуют.
  Чекаут на `master`, не detached; `local == origin`.

### Приёмки — красным-на-базе, через настоящий plan()

`bot/test_spec.py`: гейт HF параметром на трёх значениях (0.998 -> None, 0.9995 -> None,
0.9996 -> план с hf_est≈0.999566); девиационный рубеж — ОТДЕЛЬНО и с заведомо широким
HF-порогом, иначе тест судит о двух гейтах разом (поймано первым прогоном: на шаге 0.3533%
цель воду не пересекает, и «порог девиации не работает» было бы ложным выводом); арифметика
переписи старый/новый порог; цель, воду НЕ пересёкшая, не проходит ни при каком пороге ≤1.0.
Зонды и kill-switch — тест уже был на HEAD (`test_check_pending_spec_probe_revert_skips_consec_but_books_gas`), прогнан зелёным.
**Суита 253 passed. Негативный контроль:** отвязка порога от конфига в продукте роняет 3 теста,
включая приёмку — значит она стоит на настоящем гейте, а не на арифметике, переписанной в тесте.
Пин `assert C.SPEC_HF_FIRE == 0.998` снят: он описывал БОЕВОЙ порог величиной из КОДА и стал бы
ложью ровно с того момента, как боевое значение уехало в env.

### Шаг 0 решения: приз НЕ бесхозный, и поле расколото по технике

Победитель 43827262 подтверждён ранее до знака (`0x2f18fc90`, EOA `0xa8b4d313`, tip 0.097 gwei
НИЖЕ пушевых 1.129, idx2 сразу за пушем idx1, второй EOA `0x4dc3b0e8` тем же контрактом
опоздал на индекс, status=0) — перемерять не стал, это уже двойная проверка.

Что добавлено замером (офлайн по `data/shadow_races.jsonl`, в сеть не ходили):
**атомарные и реактивные — разные поля.** Контракт `0x7a0ea1f7` (сторонний слот пуша
RedStone) ходит тремя EOA, и все 15 записей — push+liquidate в ОДНОЙ tx; туда же ушла самая
жирная в выборке ($164,450, блок 43838096). Реакция такой класс не берёт по построению.
Наш класс — реактивный: 43827262 взят отдельной tx через +1 индекс после пуша, веером из
двух EOA, и один из них ПРОИГРАЛ. Значит класс берётся плотностью попыток, промахи в нём норма.

**Поправка к записи выше по файлу:** `0xae86edb5` (9 из 10 пересечений переписи) назван там
«гоняет предиктивную технику». Замер говорит иное — он атомарный self-push через белый слот:
он чужой пуш не предсказывает, он им ВЛАДЕЕТ. Конкурировать с ним предиктивным слоем нельзя
в принципе, и его 9/10 — не доказательство, что наш слой опоздал бы.

**Пределы замера.** Латентность пуш→tx по стенке внутри ОДНОГО блока не наблюдаема вовсе;
наблюдаемы техника и зазор индексов. Классифицировано 16 записей из 60: в 35 под старым
потолком разбора (12 tx) не виден ни один tx с оракул-событием («пуш не виден» ≠ «пуша не
было»), ещё 9 идут без анатомии блока. Потолок снят в 2d105c0 — доля должна вырасти на новой
телеметрии. Продублировано в `~/.fleet-watch/INFRA-FACTS.md`.

### Что ещё НЕ закрыто

* **Приёмка «телеметрия пишет свежим кодом» не может завершиться сейчас** — она требует
  СЛЕДУЮЩЕЙ чужой ликвидации пула: поля `idx_shift`/`bonus_src` появятся в записи только
  тогда. Код на диске проверен грепом, но это проверка кода, а не записи.
* **Следующий предиктивный флип = первый боевой тест слоя.** Отдельной машинерии не нужно:
  побудка shadow уже стоит. Ждём и ловим — но ждать надо РЕАКТИВНОГО пересечения; атомарное
  ничего о слое не скажет, и принимать его за отказ слоя нельзя.
* Сколько ЛИШНИХ окон открывает 0.0035 против 0.005 — не замерено. Потолок цены ограничен
  (`SPEC_MAX_PROBES=12`/окно, ~$0.001 за реверт зонда, дневной газ-кап), но сам расход зондов
  за сутки стоит снять с первой же полной суток новой телеметрии.

## 23.08 — ⛽-тревога заглушена до 30.08. Бот переведён в режим «опцион»

Владелец: «просто заглуши алерт и всё — бот просто опцион, мы не будем жечь на него недельную
квоту». Сделано ровно это, дальше по боту не работаем.

- Ручка: `HL_LOW_GAS_MUTE_UNTIL=2026-08-30` в `~/.hyperlend-bot/env`. Дефолты кода не тронуты.
  Откат — убрать строку + рестарт. Подробности и предел — `docs/ROLLBACKS.md` (23.08 + поправка).
- **Глушится КАНАЛ, не гард.** `check_balance()` по-прежнему запирает огонь при недоборе.
- Рестарт 19:18:01Z, pid 3742764, ветка master. Баннер процесса: `⛽-тревога ГЛУШЕНА до
  2026-08-30 00:00Z (гард огня не тронут)` — живой свидетель, а не запись в env.
- Суита 261 (было 255), негативный контроль по обеим половинам предиката.
- **ФАКТ, меняющий картину:** бот НЕ лежит. Баланс 0.1568 HYPE статичен, двигается ПОЛ вместе с
  базой газа. Огонь и зонды заперты ровно при `base > 31.4 gwei`; в 19:19Z база 19.26 ⇒ бот
  работал. Тревога владельца была снята при базе ~38 gwei.
- Живого сглатывания в бою ещё не наблюдали (условие сейчас ложно). Первое высокобазовое окно
  даст в `executor.log` строку `low-gas ГЛУШИЛКА` — смотреть там.
- Открыто, НЕ ТРОНУТО: 22 записи `balance check failed … rate limited` до рестарта — гард читает
  баланс только через `C.RPC_WRITE` и падает открытым. Пока падает, ⛽ не сработает и без глушилки.

## 23.08 (крон-триаж) — вскрытие 43989968: пуш в блоке ≠ гонка. Различает ПАРА (адаптер, фид)

Тревога shadow `0x795ed917…` (WHYPE→USDT0, долг $2,225, бонус $160, победитель по событию
`0x0d0ae44d`). Событие настоящее, промах неизбежен, действий по боевому боту не требуется.

**Факт по цепи** (архивный `hyperliquid.drpc.org`; позитивный контроль пройден — значения по
блокам РАЗНЫЕ, latest-подмены нет). Жертва `0x4f9ded17` на 43989967: залог $6,228.17, долг
$4,432.18, LT 0.7140, **HF = 1.003325 > 1** — на подтверждённом состоянии НЕ ликвидируема.
После 43989968: HF 1.2272, долг $2,225.28 (закрыто ~50%). Флип внутри блока: цена WHYPE
82.7834 → 83.1267 (+0.41%), `1.003325 × 82.7834/83.1267 = 0.99918`.

**Решающее — чей пуш.** В блоке ДВА пуша с ОДНОЙ И ТОЙ ЖЕ величиной HYPE = 83.1266865 и одним
timestamp пакета (1787524989):
* `idx0` — релеер `0xe08496b4` в адаптер `0x24c8964338de`, tip 76.97 gwei;
* `idx3` — сам победитель, `to 0x7a0ea1f770ee` (сторонний слот пуша HyperLend), from
  `0x84d5e28090`, tip 15.53 gwei, в адаптер `0xe4ae8874` — и `LiquidationCall` в ТОЙ ЖЕ tx.

Оракул HyperLend для WHYPE = `0xb6022412` (прокси), имплементация `0x2e65b1a4` содержит адрес
`0xe4ae8874` и НЕ содержит `0x24c89643`. Сверено значениями: на 43989967 `latestAnswer` фида =
82.7834087 = значение `0xe4ae8874`, тогда как у `0x24c89643` там 82.651734. **Пуш релеера на
idx0 оракул HyperLend не двинул вообще.** Цену в оракул завёл сам победитель — это класс
«атомарный self-push через белый слот», нам закрытый в байткоде (форк-канарейка 04–05.08).

**Урок прибора.** `push_idx = [0,1,3]` и `upd>0` соблазняют прочитать гонку как реактивную
(«перед победителем в блоке стоял чужой пуш»). Апдейт на адаптере, который оракул по данному
активу не читает, гонкой не является. И судить «по адаптеру» тоже нельзя: `0x24c89643` —
законный адаптер для ETH/UETH (см. карту в `bot/spec.py`), для другой жертвы стороны
поменяются. Честный вход классификации — **пара (адаптер, фид) против активов жертвы**.
Сделано: `_upd_sources()` пишет пару СЫРОЙ в `upd_src` каждой строки анатомии; сопоставление с
активом делает разбор, не прибор (карта пар дрейфует — прибор не должен врать вместе с ней).
Тесты: 3 новых на замеренной фикстуре блока, суита `test_shadow.py` 13 passed; негативный
контроль — снятие адреса адаптера из продукта роняет 3 теста.
**Вступает в силу при следующем рестарте executor'а** (shadow живёт в его процессе);
крон-сессия боевой процесс не трогает — режим «опцион», решение владельца 23.08.

**Бот работал штатно:** 22:41–22:43 серия `⚡ armed 0x4f9ded17… HF=1.0150→1.0033 USDT0->WHYPE
net=$+135..+152`, guard=OK, база 3.4 gwei (вооружён; ⛽-глушилка глушит канал, не гард). Его
оценка ($+141) сошлась с фактом ($160 вал). Итерации с HF<1 не существовало.

**Перепись пушей в адаптер, который читает оракул** (`0xe4ae8874`, 3000 блоков до 43991382,
разовая выборка): 6 tx от НЕЙТРАЛЬНОГО релеера `0x2327c3cdc6` прямо в адаптер + 2 атомарных
через `0x7a0ea1f7`. То есть **реактивный класс не пуст по построению** — материал для spec-слоя
есть (~8–10 чужих пушей/час). Гипотеза (n=2, не закон): атомарный оператор успевает положить
пакет в адаптер РАНЬШЕ штатного релеера, и цели в пределах одного шага девиации от HF=1
уходят атомарно, не открывая реактивного окна.

**Открытые лиды (не закрыты, боевого пути не касаются):**
* Зондов spec в 22:43 не было, хотя арифметика порогов сходилась (dev 0.41% ≥ 0.35%,
  hf_est 0.99918 < 0.9996). Выиграть было нельзя всё равно (класс атомарный), поэтому это лид,
  а не отказ. Цепной потребитель слоя ЖИВ по независимому свидетелю: `data/relayer_tips.jsonl`
  5725 строк, последняя на блоке 43991389 (пишется из `_tick_chain`). Не проверен гейтвейный
  тик — он и есть кандидат в слепые.
* **Это вскрытие НЕ считать боевым тестом spec-слоя** — STATE ждёт РЕАКТИВНОГО пересечения.
* В `prearm` регулярно `quote error: hard deadline 8.0s exceeded` (кромка остаётся без
  котировки). Доля не посчитана — отдельная работа.

---

## 2026-08-25 (крон-триаж соседней сессии, sow-watch Morpho/HyperEVM) — НАЙДЕН ЛОЖНЫЙ НОЛЬ В ИСТОРИЧЕСКИХ ЧТЕНИЯХ

Запись оставлена сессией, разбиравшей чужую тревогу; боевого тракта НЕ касалась (правило
«бота ведёт одна сессия»). Дальше — только замер и предлагаемая правка для владеющей сессии.

**Факт (позитивный контроль на цепи).** `rpc.hyperliquid.xyz/evm` — **не архивный узел** и на
исторический блок молча отдаёт состояние `latest`, БЕЗ ошибки. Один и тот же
`Morpho.market(0x64e7db7f…)` на блоках `0x2a0b134` / `0x2625a00` / `0x1c9c380`:
* `rpc.hyperliquid.xyz` → 515694293104469793950855 на ВСЕХ (= сегодняшний latest);
* `hyperliquid.drpc.org` → 273005241843430472620630 / 417262439371977850403755 /
  1054812660275195732602163 (разные, как и положено);
* `rpc.hyperlend.finance` → те же разные значения.

**Чем это бьёт по нам.** `analysis/rpc.py: Rpc.call()` выбирает endpoint через `_pick_url`
(ротация/бенч) **без учёта блок-тега**, а `Rpc.eth_call(to, data, tag)` историчен по
сигнатуре. Значит любой исторический вызов уезжает на неархивный узел с вероятностью ~1/3 и
возвращает СЕГОДНЯШНЕЕ состояние вместо прошлого — молча, без исключения, неотличимо от
честного ответа. Прямо задет `analysis/spec_gap.py`: цена (`S_PX`), конфиг резерва (`S_CFG`),
список активов (`S_LIST`), `getUserAccountData` (`S_UAD`), маска (`S_UCFG`) и
`getUserReserveData` (`S_URD`) — всё читается по `hex(blk)`. То есть часть офлайн-разборов
теней могла реконструировать «прошлое» по текущему состоянию. Докстринг `rpc.py` роли узлов
знает («drpc — full archive»), но КОД их не соблюдает.

**Предлагаемая правка (не сделана, ~10 строк, за владеющей сессией).** В `Rpc.call()`
определить историчность по блок-тегу в `params` (не `latest`/`pending`/`earliest`) и на таких
вызовах сужать пул до архивных endpoint'ов; на `latest` поведение не менять (горячий путь
исполнителя не трогается). Если все архивные забенчены — **поднимать исключение, а не
откатываться** на неархивный: иначе гард сам вернёт тот же тихий `latest` под нагрузкой.
Негативный контроль к фиксу: тот же прогон трёх блоков через пропатченный клиент обязан дать
три РАЗНЫХ числа, а `latest`-вызов — ротироваться как раньше.

**Не сделано намеренно:** `analysis/rpc.py` импортируется живым исполнителем
(`~/.hyperlend-bot/run.sh`, крон раз в минуту).
