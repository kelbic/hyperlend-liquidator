...
**Связанное узкое место каскада: PREARM_MAX=2.** Зонды spec-потребителя ходят только по
армленным, армится топ-2 кромки → ширина lag=0-каскада структурно ≤2 целей; №3+ платит
квоту 1.7-3.5с в горячий момент. Поднятие требует замера рейт-лимитов LiquidSwap → «го».
