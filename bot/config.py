...
# ВЫКЛЮЧЕН ПО УМОЛЧАНИЮ 04.08 — ФОРК-КАНАРЕЙКА ОПРОВЕРГЛА ПРЕМИССУ «пуш пермишенлесс».
# Дифференциальный тест (anvil-форк на блоке 42293069, ОДИН И ТОТ ЖЕ payload релейера,
# состояние сброшено перед каждой отправкой):
#     релейер 0x2327c3cd… -> ValueUpdate, цена 55.2124 -> 55.4922
#     релейер 0xe08496b4… -> ValueUpdate, цена 55.2124 -> 55.4922
#     наш боевой EOA      -> UpdateSkipDueToBlockTimestamp, цена НЕ изменилась
#     случайный адрес     -> UpdateSkipDueToBlockTimestamp, цена НЕ изменилась
# Отказ МОЛЧАЛИВЫЙ: status=1, tx «успешна», газ сожжён, цена на месте. В бою это значит
# гарантированный реверт ликвидации по HF (цену мы не сдвинули) — три подряд снимают бота
# kill-switch'ем в самое денежное окно. Плюс замер порога адаптера по истории событий:
# апдейты идут только при |Δцены| >= 0.50% (0.501/0.504/0.507/0.508%), интервал без апдейта
# доходил до 70 минут — то есть даже у авторизованного апдейтера мелкая правка цены не
# проходит. Включать ТОЛЬКО если доказано обратное (наш адрес попал в список апдейтеров).
SPEC_FIRE = os.environ.get("HL_SPEC_FIRE", "0") == "1"
...
# Минимальная девиация фида, при которой апдейт вообще принимается адаптером (замер по истории
# ValueUpdate: приняты 0.501/0.504/0.507/0.508%, ничего ниже 0.5% нет). Фид с меньшей девиацией
# в пуш не кладётся и масштаб от него не берётся: пуш был бы молчаливым no-op, а выстрел по
# несдвинутой цене — гарантированным ревертом. Второй рубеж после SPEC_FIRE=0.
SPEC_MIN_DEVIATION = float(os.environ.get("HL_SPEC_MIN_DEVIATION", "0.0052"))
