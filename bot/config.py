...
# 05.08, замер по 21 реальной ликвидации HyperLend за 6 суток + 19 блоков целиком:
#   * порядок по tip ПОДТВЕРЖДЁН строго: 398 пар «раньше = больше чаевых» против 93 (81%);
#     кросс-блочная корреляция idx с tip (−0.008) была негодным тестом — в разных блоках
#     разные соперники, сравнивать можно только ВНУТРИ блока;
#   * платить надо не «долю приза», а РЫНОЧНУЮ цену места: верх блока стоит медиану 2.0,
#     p75 = 5.0, максимум за окно 20.02 gwei, а победители ликвидаций платили 0.0–5.01.
# Прежняя формула (доля приза как ДРАЙВЕР) на ручейке назначала абсурд: приз $200 -> 115 gwei
# (в 5.7 раза выше всего, что рынок платил хоть раз), приз $1,000 -> 575 gwei (в 28 раз).
# Теперь драйвер — рынок (p90 чаевых последних блоков через eth_feeHistory, один дешёвый
# вызов), а доля приза осталась ПОТОЛКОМ. В каскад рынок сам поднимется (10.10: 80–1,225
# gwei), и потолок приза это позволит — на $190k приза потолок 1000 gwei стоит 0.04% приза.
# ОТКАТ: HL_TIP_MARKET=0 — прежняя формула «доля приза».
TIP_MARKET = os.environ.get("HL_TIP_MARKET", "1") == "1"
TIP_MARKET_MULT = float(os.environ.get("HL_TIP_MARKET_MULT", "3.0"))   # перебить верх блока
TIP_MARKET_BLOCKS = int(os.environ.get("HL_TIP_MARKET_BLOCKS", "20"))
TIP_MARKET_TTL = float(os.environ.get("HL_TIP_MARKET_TTL", "10"))      # кэш замера, секунд
# АСИММЕТРИЯ. Рыночная метрика ЗАПАЗДЫВАЕТ по построению (медиана p90 за 20 блоков, кэш 10с):
# в первый блок каскада соперник уже бьёт сотнями gwei, а наша медиана ещё показывает штиль.
# На ручейке это неважно (приз $50-200, проигрыш дешёвый), на ките — решает: сэкономить $80
# чаевых ценой риска потерять $190k приза это плохая сделка в обе стороны. Поэтому на крупном
# призе платим ПОТОЛОК бюджета сразу, не спрашивая рынок: 1000 gwei на $190k = 0.04% приза.
TIP_BIG_PRIZE_USD = float(os.environ.get("HL_TIP_BIG_PRIZE_USD", "5000"))
...
# История слоя. Строился как self-push (liquidateWithPush контракта v2), но форк-канарейка
# 04.08 ОПРОВЕРГЛА премиссу «пуш пермишенлесс»: адаптер принимает апдейт только от
# авторизованных релейеров (0xe08496b4…, 0x2327c3cd…). Дифференциал (anvil-форк 42293069,
# ОДИН payload, состояние сброшено между отправками): релейеры -> ValueUpdate, цена сдвинута;
# наш EOA и случайный адрес -> UpdateSkipDueToBlockTimestamp, цена на месте, status=1 —
# отказ МОЛЧАЛИВЫЙ, в бою = гарантированный реверт по HF и kill-switch в денежное окно.
# Потребитель (замер 156 пушей за 6ч, scan_pushes 04.08). Девиация гейтвея >= 0.5% ОБЯЗЫВАЕТ
# релейера запушить; пуш ложится через 2.5-7с после подписи пакета (p10=2.5с p50=6с p90=7с) —
# мы, полля тот же гейтвей раз в 1.5с, видим девиацию РАНЬШЕ лендинга. Пока окно открыто
# (девиация жива, chain не догнал гейтвей), каждый hot-проход шлёт ОБЫЧНЫЙ liquidate-зонд с
# tip=SPEC_TIP_GWEI: малые блоки упорядочены по tip убыванию, зонд с tip НИЖЕ релейерского
# в пуш-блоке ложится ПОСЛЕ пуша = ликвидация в самом блоке транзита (lag=0 — техника
# оператора, забиравшего 99.8% тихого поля). Зонд в блоке без пуша ревертится на HF-чеке за
# ~211k газа (~$0.001, форк-замер 04.08) — ожидаемая цена плотности; kill-switch их consec-
# счётчик не кормится (rec["spec"] в _check_pending), дневной газ-кап учитывает как всё.
...
# Порог предсказания = порог самого релейера: 0.50% (замер истории ValueUpdate: приняты
# 0.501/0.504/0.507/0.508%, ничего ниже). Девиация ниже — пуша не будет, окно не открывается;
# выше — пуш придёт в течение секунд. Девиационные пуши ложатся при 0.50-0.63% (HYPE max
# 0.630% за 6ч) — это и есть измеренный overshoot, на который вправе рассчитывать HF_est.
SPEC_MIN_DEVIATION = float(os.environ.get("HL_SPEC_MIN_DEVIATION", "0.005"))
# Полоса tip зонда (замер 156 пушей + 2 победы оператора 04.08): оператор всегда 0.01 gwei,
# релейер p10=0.05 p50=1.5 p90=9.9. 0.02 = ВЫШЕ оператора (в общем пуш-блоке мы раньше него),
# ниже релейера в 95.5% пушей; сэндвич (пуш с tip<0.02 — 7/156) стоит один реверт ~$0.001.
SPEC_TIP_GWEI = float(os.environ.get("HL_SPEC_TIP_GWEI", "0.02"))
# Кап зондов на окно: лендинг пуша p90=7с, зонд ~раз в блок — 12 кроет хвост с запасом;
# дальше девиация либо запушена (добита реактивом), либо ушла под порог, либо что-то не так.
SPEC_MAX_PROBES = int(os.environ.get("HL_SPEC_MAX_PROBES", "12"))
# Пауза без плана, после которой окно по цели считается закрытым (следующее — новое, со
# свежим капом зондов): пуш+реактив укладываются в секунды, 5с тишины = окно отжило.
SPEC_WINDOW_GAP_S = float(os.environ.get("HL_SPEC_WINDOW_GAP_S", "5"))
...
# kHYPE-семейство в карте DRIVERS (04.08, accessList-разбор): kHYPE/beHYPE/PT-kHYPE были вне
# карты, из-за чего предикт видел 1 позицию ближней кромки из 8 (~$584k бонуса слепы). Ноги
# доказаны бит-в-бит; асинхронность пушей (35 tx HYPE vs 17 tx kHYPE за 6ч, общих НОЛЬ) — это
# и есть окно. ОТКАТ: HL_SPEC_KHYPE=0 (карта возвращается к составу до разбора).
SPEC_KHYPE = os.environ.get("HL_SPEC_KHYPE", "1") == "1"
