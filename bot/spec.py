...
on-chain гейтами контракта. ЗАМЕР погрешности (22.08, все 10 пересечений-в-блоке за 14 суток, analysis/spec_gap.py):
максимум |HF_est − цепь| = 0.000042, то есть запас порога 0.998 держится против ошибки,
которая в ~48 раз меньше него.

ПОПРАВКА 22.08 (блок 43827262). Прежняя строка «позиция coll==debt самогасится: s_c==s_d =>
HF_est==HF => не стреляет» была НЕВЕРНА как общее утверждение — сокращение наступает только
при f_c == f_d, то есть когда пара (coll, debt) покрывает позицию целиком. У живой цели
0xaf421e57 залог был wstHYPE(62%) + USDT0 + USDC, а долг ЦЕЛИКОМ wstHYPE: f_c=0.6217,
f_d=1.0000, и общий пуш HYPE +0.5039% опустил HF 1.001465 -> 0.999608 (цепь). Формула это
считает правильно (HF_est=0.999566) — неверен был только комментарий; код по этому признаку
никогда не выходил досрочно и выходить не должен. Регрессия:
bot/test_spec.py::test_plan_same_asset_does_NOT_self_cancel_when_shares_differ.
"""
